...
if(!collectionExists) {
	...
	def projection = Projections.fields(
		...
		// Prints out the select columns only iff they have no Twitter ID and Youtube ID
		def printSenatorsWithNullTwitterId(projectedResult, cache) {
			def cacheKey = "printSenatorsWithNullTwitterId"
			...
			projectedResult.each { document ->
				if (document.get("Twitter ID") == null && document.get("YouTube ID") == null) {
		...
		for (int i = 0; i < numberOfRuns; i++) {
			...
			printSenatorsWithNullTwitterId(projectedResult, cache)
